...
def distortion(H1, H2, n, jobs):
    ...
    dists = Parallel(n_jobs = jobs)(delayed(distortion_row)(H1[i, :], H2[i, :], n, i) for i in tqdm(range(n), total = n, desc = 'Obtaining distortion'))
...
def map_score(H1, H2, n, jobs):
    ...
    maps = [map_row(H1[i, :], H2[i, :], n, i) for i in tqdm(range(n), total = n, desc = 'Obtaining map score')]
