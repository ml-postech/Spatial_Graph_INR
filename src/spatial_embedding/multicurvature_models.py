...
from src.spatial_embedding.multicurvature_parameter import PoincareParameter, EuclideanParameter, SphericalParameter, HyperboloidParameter
...
class Hyperbolic_Mean(nn.Module):
    def __init__(self, d):
        ...
        self.w = HyperboloidParameter(sizes = d)

    def loss(self, y_data):
        return torch.sum(torch.dist(self.w.repeat(y_data.size(0),1), y_data) ** 2)
...
class ProductEmbedding(nn.Module):
    def __init__(self, n, hyp_d, hyp_copies = 1, euc_d = 1, euc_copies = 0, sph_d = 1, sph_copies = 0,
                 project = True, initialize = None, learn_scale = False, initial_scale = 0.0,
                 absolute_loss = False, logrel_loss = False, dist_loss = False, square_loss = False, sym_loss = False,
                 exponential_rescale = None, riemann = False):
        super().__init__()
        self.n = n
        ...
        print('Sizes for spatial graph embeddings')
        ...
        exp_str = f'Exponential with rescale factor of {self.exponential_rescale}' if self.exponential_rescale is not None else 'None'
        logging.info(f'Loss: {abs_str}, Rescale: {exp_str}')

    def step_rescale(self, values):
        y = mds_warmstart.cudaify(torch.ones(values.size()).double() / (10 * self.n))
