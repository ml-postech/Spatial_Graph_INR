...
import src.spatial_embedding.utils.load_graph as load_graph
import src.spatial_embedding.utils.vis as vis
import src.spatial_embedding.utils.distortions as dis
import src.spatial_embedding.utils.graph_helpers as gh
import src.spatial_embedding.utils.mds_warmstart as mds_warmstart
...
print(f'Device used for getting spatial graph embedding: {device}')
...
def cu_var(x):
    if isinstance(x, list):
        return [cu_var(u) for u in x]
    if isinstance(x, tuple):
        return tuple([cu_var(u) for u in list(x)])
...
def unwrap(x):
    ...
    if isinstance(x, list):
        return [unwrap(u) for u in x]
    if isinstance(x, tuple):
        return tuple([unwrap(u) for u in list(x)])
...
class GraphRowSubSampler(torch.utils.data.Dataset):
    def __init__(self, G, scale, subsample, weight_fn, Z = None):
        ...
        # initialize up front
        for i in tqdm(range(self.n), total = self.n, desc = 'Subsampling'):
            self.__getitem__(i)
    ...
    def __repr__(self):
        return f'Subsample: {self.n} points with scale = {self.scale} and subsample = {self.subsample}'
...
# Output: Z = Distance matrix of graph with unit edge lengths, z = DataLoader created from Z with (idx, distance, (weight))
def build_dataset(G, lazy_generation, sample, subsample, scale, batch_size, weight_fn, num_workers):
    ...
    if lazy_generation:
        ...
        logging.info('Dataloader successfully bulit')
...
# model params
@argh.arg("--hdim", help = "Hyperbolic dimension to use")
...
def learn(dataset, hdim=2, hyp=1, edim=1, euc=0, sdim=1, sph=0, scale=1., riemann=False, learning_rate=1e-1, decay_length=1000, decay_step=1.0, momentum=0.0, tol=1e-8, epochs=100, burn_in=0,
    ...
    logging.basicConfig(level = logging.DEBUG, format = '%(asctime)s %(message)s', datefmt = '%F %T ',)
    
    if log_name is None and log:
        log_name = f'{os.path.splitext(dataset)[0]}.H{hdim}-{hyp}.E{edim}-{euc}.S{sdim}-{sph}.lr{learning_rate}.log'
    ...
    logging.info(f"Loaded Graph {dataset} of {n} nodes with scale = {scale}")
    ...
    if model_load_file is not None:
        ...
        if warm_start:
            ...
            m_init = torch.DoubleTensor(mds_warmstart.get_model(dataset,hdim,scale)[1])

        if warm_start:
            logging.info(f"\t Warmstarting? - {warm_start} {m_init.size() if warm_start else None} with {G.order()} nodes")
        else:
            logging.info(f"\t Warmstarting? - {warm_start} with {G.order()} nodes")
        ...
        m = ProductEmbedding(G.order(), hdim, hyp, edim, euc, sdim, sph, initialize=m_init, learn_scale=learn_scale, initial_scale=initial_scale, logrel_loss=logloss, dist_loss=distloss, square_loss=squareloss, sym_loss=symloss, exponential_rescale=exponential_rescale, riemann=riemann).to(device)
        m.normalize()
        
        m.epoch = 0
        
    logging.info(f'Constructed model with epochs = {m.epoch} and isnan = {np.any(np.isnan(m.embedding().cpu().data.numpy()))}')
    logging.info(f'Model info - Hyp: Dim = {hdim}, Copy = {hyp} / Sph: Dim = {sdim}, Copy = {sph} / Euc: Dim = {edim}, Copy = {euc} / Use Riemann matrix = {riemann}')

    if visualize:
        name = 'animations/' + f'{os.path.split(os.path.splitext(dataset)[0])[1]}.H{hdim}-{hyp}.E{edim}-{euc}.S{sdim}-{sph}.lr{learning_rate}.ep{epochs}.seed{seed}'
